...
Application::Application() {
	...
	// Ground body
	{
		...
		b2EdgeShape shape;
		shape.Set(b2Vec2(-50.0f, 0.0f), b2Vec2(955.0f, 0.0f)); // 1km long ground j'ai fait une petite translation en arrière
...
void Application::checkv1(bool statut) {
 m_biped->getStateMachine()->manualspeed = statut;

 }

 void Application::setv1(float v1) {

     m_biped->getStateMachine()->v1 = v1;
...
void Application::interpolation(bool statut){

        m_biped->getStateMachine()->m_interpolation = statut;
}

void Application::Virtualforce(b2Vec2 force) {

    m_biped->applyVirtualForce(force);

}


void Application::AngleLimite(float angle) {

	//m_biped->seuil = angle;
	//Passage degré au radian
	m_biped->seuil = angle*b2_pi/180.0f;
	//m_biped->seuil = angle*180/b2_pi;
}

void Application::zeroenergy(bool m) {
    m_biped->energy0 = m;
}



void Application::fsmchange(bool statut) {

m_biped->switcher = statut;
...
void Application::Walk() {
    if (m_biped->Biped::getFSMname() == "Walk") {
        m_biped->changeFSM(new FSM_MoonWalker());
    } else {
        m_biped->changeFSM(new FSM_Walk());
    }
}

string Application::FSMname() {
    return (m_biped->Biped::getFSMname());
...
void Application::setOptimizationData(const float * data) {
    /*
    Dans cette procédure on affecte aux paramètres à optimiser le contenu du tableau data
    Ex. m_biped->getStateMachine()->m_states[0].targetAngles[0] = data[0];
    pour affecter la première valeur du tableau au premier angle du premier état de la machine à états
    ou */
    float kpkd = data[4];
    m_biped->getPDControllers()[CHEVILLE_GAUCHE]->setGains(data[0], data[0]*kpkd);
    m_biped->getPDControllers()[GENOU_GAUCHE]->setGains(data[1], data[1]*kpkd);
    m_biped->getPDControllers()[HANCHE_GAUCHE]->setGains(data[2], data[2]*kpkd);
    m_biped->getPDControllers()[NB_ARTICULATIONS]->setGains(data[3], data[3]*kpkd);

    // Configurer les contrôleurs symétriques avec les mêmes gains
    m_biped->getPDControllers()[CHEVILLE_DROIT]->setGains(data[0], data[0]*kpkd);
    m_biped->getPDControllers()[GENOU_DROIT]->setGains(data[1], data[1]*kpkd);
    m_biped->getPDControllers()[HANCHE_DROIT]->setGains(data[2], data[2]*kpkd);
    /* pour les gains Kp et Kv du premier PD controller
    */
}

void Application::getOptimizationData(float * data) {
    /*
    Dans cette procédure on affecte dans le tableau les valeurs des paramètres à optimiser
    Ex. data[0] = m_biped->getStateMachine()->m_states[0].targetAngles[0];
    pour affecter la valeur du premier angle du premier état de la machine à états à la première valeur du tableau
    ou */


    //

    data[0] = m_biped->getPDControllers()[CHEVILLE_GAUCHE]->getKpGain();
    data[1] = m_biped->getPDControllers()[GENOU_GAUCHE]->getKpGain();
    data[2] = m_biped->getPDControllers()[HANCHE_GAUCHE]->getKpGain();
    data[3] = m_biped->getPDControllers()[NB_ARTICULATIONS]->getKpGain();
    data[4] = m_biped->KpKvRatio;
	...
	if (settings->steps) {
	    ...
	    if (needNewStepBoxes) {
            ...
            for (int i=0;i<nbSteps;i++) {
                ...
                float hauteurMarche = RandomFloat(0.01,0.2);
                b2BodyDef bd;
...
////////////////////////////////////////////////////////////////////////////////////
//void Application::optimizeTransition() {
    // Exemple simplifié. Vous aurez besoin d'adapter cette logique en fonction de vos besoins spécifiques.
  //  float minimalCost = std::numeric_limits<float>::max();
    // Essayez différents états initiaux pour la nouvelle FSM et calculez le coût
    //for (int i = 0; i < m_biped->getStateMachine()->getNumberOfStates(); ++i) {
      //  m_biped->getStateMachine()->setCurrentState(i);
        //float cost = getCurrentCost();
        //if (cost < minimalCost) {
          //  minimalCost = cost;
            //* Cette condition détermine le meilleur état initial
